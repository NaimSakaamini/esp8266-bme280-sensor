...
#define ESPNOW_CHANEL CONFIG_ESPNOW_CHANEL
...
static void espnow_init()
{
	tcpip_adapter_init();
		...
		ESP_ERROR_CHECK(esp_wifi_set_channel(ESPNOW_CHANEL, 0));

	ESP_ERROR_CHECK(esp_now_init());
	ESP_ERROR_CHECK(esp_now_register_send_cb(esp_now_send_cb));

	station_peer.channel = ESPNOW_CHANEL;
	station_peer.ifidx = ESP_IF_WIFI_STA;
	station_peer.encrypt = false;
	const char *mac = ESPNOW_STATION_MAC;
	for (uint8_t i = 0; i < ESP_NOW_ETH_ALEN; i++)
	{
		station_peer.peer_addr[i] = (uint8_t) strtol(mac + (3 * i), (char **) NULL,
				16);
	}
	ESP_LOGI(LOG_TAG, "Station MAC %02x:%02x:%02x:%02x:%02x:%02x",
			station_peer.peer_addr[0], station_peer.peer_addr[1],
			station_peer.peer_addr[2], station_peer.peer_addr[3],
			station_peer.peer_addr[4], station_peer.peer_addr[5]);
...
static espnow_sensor_data_t fetch_data()
{
	...
	ret.battery = ((vcc > MAX_VCC ? MAX_VCC : vcc) - MIN_VCC)
			/ (float) (MAX_VCC - MIN_VCC) * 100;

	ESP_LOGI(LOG_TAG, "Battery voltage %d mV", vcc);
	ESP_LOGI(LOG_TAG, "Battery capacity %d %%", ret.battery);
	return ret;
}
